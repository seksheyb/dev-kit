...
190 entries total. No plugin boundaries changed; this is a reading guide layered on
top of the existing marketplace structure (see the [root README](../../README.md) for
install instructions).

The `dk` orchestration plugin (commands, hooks, and the RUNBOOK that sequences the
whole pipeline) and the model-routing layer are documented separately in
`docs/guide/pipeline.md` and `docs/guide/architecture.md` — this catalog covers the
capability plugins (`dev-kit-core` and the 7 lane plugins) only.
...
|---|---|---|---|
| Product / Requirements Owner | Turning an idea into a validated, unambiguous spec | [core-discovery-and-design.md](core-discovery-and-design.md#role-product--requirements-owner) | 16 |
...
| Engineer (Build & Debug) | Implementing, debugging, and verifying the plan against reality | [core-build-and-ship.md](core-build-and-ship.md#role-engineer-build--debug) | 25 |
| Security / Compliance Reviewer | Checking the implementation against threat models and regulatory bars | [core-build-and-ship.md](core-build-and-ship.md#role-security--compliance-reviewer) | 10 |
...
**94 items**: 49 skills, 37 agents, 8 commands.
...
**96 items**: 92 lane skills + 4 `dev-kit-data-ai` agents (the only agents in the lane
plugins; every other lane plugin is skills-only).
