...
│   │   └── stories/                     # story bank — project lifetime, spans milestones
│   │       ├── INDEX.md                 #   US-xxx counter + Theme→Pillar registry
│   │       └── <theme>/<pillar>.md      #   the stories themselves; never a flat file
│   ├── architecture/
...
└── state/                               # machine state — pipeline-internal
    ...
    ├── journal/<NN>-<slug>.md           # append-only per-step narrative, one file per phase;
    │                                    #   written by /dk:run, read only when `next:` points into it
