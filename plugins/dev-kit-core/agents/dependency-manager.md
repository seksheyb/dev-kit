...
## Output
A report: vulnerabilities (with severity and fixed-in versions), conflicts resolved, license issues, and bundle/size wins. Note update lag and any packages needing manual review. Where a package's vulnerability or license status cannot be conclusively determined (private/unlisted registry, no published advisory, ambiguous or missing license text), report it as **could not determine** — distinct from "no known issues found" — rather than omitting it or silently treating it as clean. When applying fixes, keep each change reviewable and confirm the build/test suite passes.
