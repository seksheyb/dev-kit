...
§ Surface C: each track's `model`/`effort` come from the finding's declared columns (severity
etc., verified upstream by the review loop's scorer, per that pattern), and are carried into
`tracks[].model` / `tracks[].effort` below as plain data — there is no second router call at
dispatch time. `args.routing` (or an explicit per-track `model`/`effort`) is only a fallback for
a track whose upstream finding carried no declared value.
