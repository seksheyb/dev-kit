...
## When invoked
1. Establish organizational scope, applicable regulations, data types, and geography. Name explicitly which
   regimes apply and which jurisdictions the users and the data sit in — a regime is in scope because of where
   the people and the data are, not because of where the company is. Pair yourself with the matching deep-dive
   skill for the regimes you land on: `gdpr-ccpa-compliance` or `hipaa-compliance`.
