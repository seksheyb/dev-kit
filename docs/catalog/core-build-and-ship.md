...
- **What it does:** Enforces a four-phase root-cause-first process — investigate (reproduce, bisect, gather evidence), pattern-match against known bug signatures, form and test falsifiable hypotheses — one at a time by default, or one read-only investigator fanned out per hypothesis when several genuinely compete (3-strike rule before questioning architecture), then implement a single minimal fix backed by a failing-then-passing regression test. Closes with a structured DEBUG REPORT (symptom/root cause/fix/evidence/status).
...
- **What it does:** Covers three things: dispatching a reviewer subagent with precisely-crafted context (never the requester's full session history) at mandatory checkpoints (after each task, after major features, before merge); when receiving feedback, a read/understand/verify/evaluate/respond/implement pattern that forbids gratitude-performance and requires checking suggestions against the actual codebase before acting, including a YAGNI check on "implement this properly" suggestions; and the multi-lens review round — freeze a commit, fan every finder out against that one tree, triage into a single deduped findings set, then one fix sweep, on the rule that every finder is read-only and only the sweep writes.
...
- **When to use:** Dispatched by the orchestrator/pipeline to audit an implemented phase against its threat model; falls back to `security-reviewer`'s methodology directly when no threat model exists. Checks for `cso`'s latest `.security-reports/` entry first (Stage 12 fans out one of these per phase and runs `cso --diff` only once they have all returned — never beside them, since cso needs the whole merged threat register to diff against) and treats it as available scan-tool evidence rather than re-running the same tools. Also backs the `/security-audit` command.
...
- **What it does:** Parses `$ARGUMENTS` for an optional scope (paths, subsystem, or focus area; default: the whole repo or current change) and dispatches the `security-auditor` agent over that scope. It also owns the milestone-wide fan-out `/dk:final:gate` calls at Stage 12: one auditor per phase scoped to that phase's own declared threat model, routed through `security-gate.workflow.mjs` at 2+ phases, plus the barrier rules that keep `cso` after every auditor and `dependency-manager` after `cso`.
