...
**Parallel verification:** for each candidate finding, launch an independent verification subagent with fresh context — give it only the file path + line number and the FP rules, and ask it to independently score 1-10. Route by how many candidates survived the hard-exclusion list above. Anything this route cannot independently confirm falls back to a distinct `SELF-VERIFIED` status, never `VERIFIED` — see the fallback rule below the dispatch block. This is not a preference:

| Candidates remaining | Route |
|---|---|
...
Workflow({
  ...
  args: {
    findings: [                    // required, 2 or more — candidates surviving hard exclusions
      { id, title, file, line, category, severity }, // one entry per candidate finding
    ],
    fpRules: "<verbatim hard-exclusion list + precedents text from this phase>", // required —
      // pre-extract this yourself; the subagent gets ONLY file:line plus this text, never
      // the finding's title/category/severity, so its score stays independent
    gateThreshold: 8,              // required — 8 for daily mode, 2 for comprehensive mode
  },
})
```
...
The script has zero judgment: each verifier independently scores its one finding 1-10 against the FP rules you passed, then a pure mechanical gate keeps `score >= gateThreshold` and discards the rest. It returns `{ gateThreshold, verified: [...], discarded: [...], unverified: [...] }` — every entry in all three arrays echoes that finding's `id/title/file/line/category/severity`; `verified` and `discarded` entries also carry `score`, `reasoning`, and `evidenceQuoted` from the verifier. `unverified` holds any finding whose verifier subagent returned nothing at all (died, was skipped, or failed schema validation) — that finding was neither confirmed nor cleared, and it never collapses into `discarded`.

For every finding in `unverified`, and whenever the Workflow route is unavailable entirely (exactly 1 candidate, or no subagent dispatch available), self-verify by re-reading the code with a skeptic's eye — but mark that finding's Status as `SELF-VERIFIED`, not `VERIFIED`, in Phase 13/14 output. A same-context re-read shares the reasoning blind spots that produced the finding in the first place; it must never read identically to a finding an independent subagent actually confirmed.
