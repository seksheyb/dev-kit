...
Copy `${CLAUDE_PLUGIN_ROOT}/bin/complexity-score.mjs` and `${CLAUDE_PLUGIN_ROOT}/bin/complexity.config.json`
to `.claude/bin/` on the same terms — that is where the step-7 plan gate looks for the scorer.
