...
description: Use when analyzing A/B test results, interpreting p-values, determining statistical significance, or making a ship/no-ship decision. Invoke for experiment result interpretation, confidence intervals, guardrail metric checks, and catching statistical traps like peeking or p-hacking. Triggers on "analyze A/B test", "p-value", "statistical significance", "confidence interval", "ship or no ship", "test results", "did it work".
license: MIT
metadata:
  version: "1.0.0"
  domain: product-analysis
  triggers: A/B test, p-value, statistical significance, confidence interval, ship decision, experiment results, lift, guardrail metrics
  role: analyst
  scope: analysis
  output-format: report
  related-skills: cohort-analysis, assumption-mapping
---

# A/B Test Analysis

Statistically principled A/B test interpretation and ship/no-ship decisions. Correctly interpret experiment results, catch common analysis errors, and act on data without falling for statistical traps.
...
Only report segments that were pre-planned — post-hoc segmentation is p-hacking.
