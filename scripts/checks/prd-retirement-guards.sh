...
# Check 6 (the pipeline walkthrough drops the milestone-1 PRD special case) was removed when
# docs/gwd-pipeline-on-devkit.md was deleted. That doc was a design-rationale artifact for the
# pipeline build, not a shipped asset, and the PRD retirement it recorded is now asserted where
# it is actually load-bearing: the sitemap Retired-paths rows (check 1) and the specify /
# gate-reverse-engineer / doc-synthesizer / doc-classifier assets (checks 2-5). Do not re-add a
# check against a deleted doc.
