...
## Method
Apply the skill's Test-and-classify step: validate every finding via safe proof-of-concept; assess real impact; document evidence with reproduction steps. Verify before reporting — no false positives, no speculative severity. If a test could not be run to conclusion within scope (blocked, out of window, target unreachable) or exploitability couldn't be conclusively confirmed, that is a distinct outcome from "not vulnerable" — mark it, do not guess a severity for it and do not drop it.

## Output
A report following the skill's `references/report-template.md`: executive summary, technical findings with proof-of-concept and file:line/endpoint refs, severity ratings (critical/high/medium/low/info) with likelihood + impact, each finding's Confirmation status (Confirmed — validated via proof-of-concept, or Needs Verification — exploitability/impact could not be conclusively established within scope; never collapse this into "Confirmed" or omit the finding), and prioritized remediation (quick wins → strategic fixes). Include compliance mapping and retest guidance. Practice responsible disclosure throughout.
