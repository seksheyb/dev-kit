...
1. **Form Single Hypothesis**
   ...
   - If Phase 2 left several equally plausible causes and no evidence separates them, don't pick one on a hunch — see **Parallel Hypothesis Fan-Out** below
...
**WHEN the failure has several plausible root causes and no evidence yet separates them** — the timeout could be the connection pool, the retry wrapper, or clock skew on the signing host — testing them one at a time is serial guesswork. Each miss costs a full investigation cycle, and the order you happen to try them in decides how long the bug lives.

Instead, dispatch one investigator per hypothesis, **all in a single message so they run in parallel**. Give each one:

- **The same symptom** — identical reproduction steps, error output, and Phase 1 evidence for every investigator, so their verdicts are comparable rather than each answering a slightly different question.
- **Exactly one hypothesis** — stated as the falsifiable claim from step 1 above. One hypothesis per investigator, never a list.
- **The task: confirm it or kill it, with evidence.** file:line references, log output from an added assertion, a diff that reproduces or fails to reproduce. A verdict of KILLED must be reported as loudly and as specifically as CONFIRMED — "ruled out because the pool never exceeded 4 of 20 connections during the failure window" is a result, "probably not this" is not.

**The investigators are READ-ONLY.** They diagnose; they do not fix. No edits, no "while I was in there" cleanups, no speculative changes to see what happens. This is not politeness — N investigators editing concurrently produce a working tree nobody can attribute, and a fix from an investigator whose hypothesis later loses is a change with no rationale behind it. Temporary instrumentation, if a hypothesis truly can't be tested without it, is reverted before the investigator reports. **Only the single convergent fix writes code, after one hypothesis has won**, and it goes through Phase 4 in full — failing test first, minimal diff, fresh verification.

**Converging on the survivor:**

| Outcome | What it means | What to do |
|---------|---------------|------------|
| Exactly one confirmed | That is your root cause | Proceed to Phase 4. Write the one fix. |
| Several confirmed | The hypotheses weren't independent, or the evidence bar was too low | Ask what would disprove each survivor, or gather the discriminating evidence yourself. Do NOT fix two things. |
| None confirmed | The obvious candidates are eliminated — cheaply, and in one round | Return to Phase 1 with what they ruled out. This counts as one strike, not N. |

**When NOT to fan out:** a symptom with one obvious cause. If the stack trace names the line, if the regression is one commit, if you can state a single hypothesis with real confidence — investigate it directly. Fan-out on a one-cause bug is pure overhead: N investigators, N reports, and N-1 of them saying "not this." The technique earns its cost only when the hypotheses genuinely compete.
...
| **3. Hypothesis** | Instrument, test minimally, fan out on competing hypotheses, 3-strike rule | Confirmed or new hypothesis |
