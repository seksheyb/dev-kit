...
3. Ask the model-selection question from the protocol once, up front — non-interactively, default
   to Sonnet without blocking (`claude-design-mcp-protocol.md` Step 1). Every subagent dispatch for
   this invocation (initial build, every refinement round, and every page in a Batch Mode run) uses
   that same chosen model, passed as `operatorModel`/`model` — never omitted, never left to the
   session default.
...
- report back exactly these keys: `project_id`, `paths` (array of every path written), `open_url`
  (never `serve_url`), `model_used`
...
## Batch Mode (initial builds only)

Pages are isolated by construction — each gets its own Implementation Spec (Step 1) and its own
output file — except for one shared mutable file: `.claude/design/screens.json`, which every
invocation upserts. Step 4's refinement loop is an interactive human back-and-forth and cannot be
parallelised. Given that, the one-page rule bends **only for the initial build**: when the user
asks for N pages at once, dispatch N Step-3 page-builders, one per page, each with its own
Implementation Spec and screen name, all bound to the same `claude_design_system_id` from Step 0
and the same chosen model.

The route is decided by page count, the same as everywhere else in this corpus:

| Pages requested | Route |
|---|---|
...
existing project id if evolving, Claude Design workflow instructions, the AI-slop blacklist, the
report-back instruction), then hand the whole batch over in one call:

```
Workflow({
  scriptPath: "<dev-kit-core>/references/workflows/design-batch.workflow.mjs",
  args: {
    pages: [
      { name: "landing-page", prompt: "<the fully-rendered Step-3 prompt for this page>" },
      { name: "dashboard", prompt: "<the fully-rendered Step-3 prompt for this page>" }
    ],
    operatorModel: "<the model chosen at Step 0's answer, or \"sonnet\" when non-interactive per claude-design-mcp-protocol.md Step 1>"   // always passed — forwarded verbatim
  }
})
```
...
written, `open_url`, model used). **No subagent touches `.claude/design/screens.json` — not the
script, not any page-builder.** The script has no filesystem access at all; the orchestrator (you)
performs every `.claude/design/screens.json` upsert yourself, serially, over the returned
`results[]`, after this run returns — so the one file all pages share has exactly one writer, in
your turn, not inside the fan-out.

A dropped page (in the return value's `dropped[]`) was not built and has no project id to upsert
— re-dispatch it before treating the batch as done. `resumeFromRunId` re-invokes a dead run:
`Workflow({ scriptPath, resumeFromRunId: "<runId>" })`.

Refinement stays serial and human — Step 4's loop is explicitly out of scope for this fan-out:
once the whole batch lands, run Step 4 per page, one page at a time, never in parallel.
...
- **One page per invocation.** For multi-page designs, run once per page — Batch Mode relaxes
  this for initial builds only (see "Batch Mode (initial builds only)" above); the refinement
  loop is always one page at a time.
