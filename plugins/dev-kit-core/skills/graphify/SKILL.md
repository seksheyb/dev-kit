...
/graphify <path> --intel                              # also write structured codebase intel (stack/files/apis/deps/arch) to graphify-out/intel/
...
Write five files to `graphify-out/intel/`, using the Write tool (never heredocs):
...
- **Never read or include** `.env` files (except `.env.example`), `*.key`/`*.pem`/`*.p12`,
  ...
  `node_modules/`, `.git/`, `dist/`, `build/`, and `graphify-out/` itself from analysis.
...
After writing, print one line: `Intel: 5 files written to graphify-out/intel/` and mention
