...
**Model routing (Surface C — no batch router call here).** Per references/model-routing.md
§ Surface C: each track's `model`/`effort` come from the plan's declared Signal Block columns,
already verified against the deterministic scorer by `gate-plan-review` before this wave is
trusted for execution — that is the authoritative source for `tracks[].model` /
`tracks[].effort` below, and there is no second router call at dispatch time. `args.routing` is
only a fallback for a track whose plan row carries no declared value.
