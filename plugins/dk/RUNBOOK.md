...
locations)* — `/dk:bootstrap:intake <M>`. When both conditions hold, the two recoveries run beside
...
*(only if this phase builds an AI/LLM system needing an eval contract, or has UI work)* —
`/dk:spec:phase <M> <NN> <NNN>`

*(if this phase has both, the two chains run concurrently inside it — they share no files. Each
...
A round is four stages — freeze, fan out the round's finders, triage into one deduped set, one fix
sweep. The finders are all read-only and the sweep is the only writer, which is what lets them share
a round. `code-review-protocol` part 3 owns the stage shape.

`/dk:review:cycle <NN> <branch>` — the test framework is settled **first**, before any round opens:
settling one is a write, so it cannot share a round. Then the ≤6-round loop. Round 1 runs every
applicable lens in the same finder stage, the UI lens among them when this phase shipped UI; from
there the rounds shrink — a lens that came back clean sits the next round out. The loop closes only
on a full-roster round that comes back all-clear, so a narrowed clean round buys one final full
round rather than an exit. **Operator decision** — a closing round that carries only advisory
P3/P4 findings stops and asks whether to sweep or accept them; it neither loops again nor exits on
its own. It never opens a 7th round.
...
## 11. Verify the goal
`/dk:verify:phase <NN>` — one read-only wave carries the authoritative goal verification (step 10's
finder stage ran a verifier only as early warning, against the pre-sweep tree), the eval audit *(only
if this phase built an AI/LLM system with an eval contract from step 6)*, and the integration check
*(skipped on the milestone's first phase)*. Converge follows that wave rather than joining it, and
runs even when verify came back `passed`; the Nyquist audit runs beside converge *(only if verify
reported validation gaps)*.

*(only if verify came back `human_needed`)* — `/dk:verify:human`. **Blocking** — `human_needed` is
never a pass.

*(only if converge appended convergence tasks, verify came back `gaps_found`, the eval audit returned a
BLOCKER, or the integration check ESCALATED an item)* — `/dk:verify:remediate`. If the same gaps survive
two cycles, it escalates.
...
`/dk:final:gate <URL>` — always runs, **blocking**.

The per-phase security fan-out always runs, and it runs **beside** the three conditional lanes — UI
*(only if the milestone shipped UI in any phase)*, devex *(only if it shipped a developer-facing
surface — API/CLI/SDK)*, and compliance *(only if regulated data or industry is in scope —
GDPR/HIPAA/PCI/SOC2)*. The command asks you which apply. Pentest *(only if active exploitation is
authorized in writing)* waits on nothing but the UI lane releasing the app URL. `cso` is the barrier
that needs the whole register back; open threats block the ship.
...
Mutually exclusive; the choice determines the changelog half of step 14's `sync`, plus `land` and
`merge`.
...
`/dk:docs:sync` — its changelog half fires *(only if you took the manual path at step 13)*, and it is
the one thing in this step that runs **alongside** the chain rather than after it: it writes the
changelog and the version file, which nothing in the chain reads or touches. `document-release`
below is the barrier that needs both back.
...
*(only if the retro surfaced a workflow this project keeps repeating by hand, or the postmortem named
a failure mode nobody had rehearsed)* — `/dk:close:followups`, placed here so both triggers have had
a chance to fire.
