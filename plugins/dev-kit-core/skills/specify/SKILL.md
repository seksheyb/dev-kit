...
- **Mode A — Direct generation**: The user gave a reasonably concrete feature
  description. Generate the spec directly (steps 1–9 below), using informed defaults and
...
- **Mode B — Interrogation (vague intent → executable spec)**: The request is a
  ...
  five-phase interrogation first, then write the spec through Mode A steps 1–9.
...
| Multi-domain pre-discovery | `references/pre-discovery-subagents.md` | Feature spans 3+ system layers in an unfamiliar codebase, before Mode A step 8 or Mode B Phase 3 |
...
3. **Assigning US-xxx IDs (global allocation, from the story bank)**:

   The registry is `docs/global/requirements/stories/INDEX.md` — project lifetime, spanning
   milestones. It owns the counter; you allocate from it and write it back.

   - **If the bank does not exist yet**, create it now: copy this skill's
     `story-bank-index-template.md` to `docs/global/requirements/stories/INDEX.md`. On a
     brand-new project that is the whole bank — pillar files come below, as stories are written.
   - **Read `**Next US number:**` from INDEX.md** and allocate this feature's stories from there,
     consecutively. Write the new value back in the same edit that adds the stories. The counter
     only ever moves forward.
   - **Never re-derive the counter by scanning `spec.md` files.** A spec that is deleted, archived
     or moved takes its high-water mark with it, and the next allocation then re-issues a retired
     ID — silently breaking every roadmap row, plan, review and `US-xxx` cross-reference that
     already pointed at it. Scan specs **only** to bootstrap the counter the first time, when
     INDEX.md is being created in a repo that already has specs: take the highest `US-\d+` found
     anywhere, add one, and record that as `**Next US number:**`. Say in your summary that you did
     this, since a spec deleted before that bootstrap is unrecoverable and its ID may be reissued.
   - **IDs are never renumbered or reused** — if a story is later deleted, split, or moved to a
     different pillar, its number is retired, not recycled. Record it under INDEX.md's
     `## Retired IDs` so it is visible that the number was spent.

4. **Placing stories in the bank (Theme → Pillar)**:
   - Ask (or infer from context) which **pillar** each story belongs to, and which **theme** owns
     that pillar. A pillar is a slice of user-facing capability, never a layer — apply
     `@references/vertical-slice.md`'s test one level up: if you cannot finish "a user can now
     ___" with the pillar's name, it is a layer, and the story belongs in a real pillar instead.
   - Write the story into `docs/global/requirements/stories/<theme>/<pillar>.md`, creating it from
     this skill's `story-bank-pillar-template.md` if it does not exist, and add or update that
     pillar's row in INDEX.md's Themes and Pillars table.
   - Fill the story's `**Pillar**:` field in `spec.md` to match. The spec restates the story for
     readability; **the pillar file is the source of truth for its wording**, so when the two
     drift, the bank wins.
   - **Flat projects**: a single-spec project with no second theme in sight may skip the pillar
     files and keep INDEX.md as a bare counter, leaving `**Pillar**:` out of the spec. Do not
     invent a one-pillar hierarchy to satisfy the shape. But still allocate from INDEX.md — the
     counter is what makes IDs safe, and it costs nothing to keep it from the start.

5. Load the spec template to understand required sections.

6. **IF EXISTS**: Load the project constitution for principles and governance constraints.

7. **For multi-domain features** (touches 3+ distinct system layers, unfamiliar codebase):
...
9. Write the specification to `SPEC_FILE` using the template structure, replacing
