 ...
 * =============================================================================
 * SURFACE C (`references/model-routing.md` §7): the plan's declared per-track `model`/
 * `effort` are authoritative at execution time — there is no fresh router call per
 * dispatch here. `args.routing` (see ARGS CONTRACT) is accepted only as the FALLBACK arm:
 * spread first into each track's opts, then the track's own declared `model`/`effort`
 * win when present. Model/effort routing for the fallback arm is decided caller-side by
 * `model-route.mjs` and arrives as `args.routing` data; the plan-declared values remain
 * the primary source this script has always used.
 * =============================================================================
 ...
 *                              declared values stay authoritative per SKILL.md/gate-
 *                              plan-review). A missing key or a missing `routing` arg
 *                              leaves `model`/`effort` absent entirely (inherit), never
 *                              defaulted.
...
function buildOpts(track) {
  const opts = {
    // SURFACE C FALLBACK ONLY (see header): the routing entry is spread first so the
    // plan-declared `track.model`/`track.effort` below always win when present. Absent
    // `routing` (or absent `track` key) contributes nothing here.
    ...(args.routing?.track ?? {}),
