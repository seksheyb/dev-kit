...
`/dk:discover:map <NN>` — five agents fan out safely.

`/dk:discover:research <NN>`
...
*(only if this phase ships UI)* — `/dk:build:ui`
...
*(only if this phase shipped UI)* — `/dk:review:ui <NN>` — design-reviewer comes at step 12.

## 11. Verify the goal
`/dk:verify:goal <NN>`

*(only if this phase built an AI/LLM system with an eval contract from step 6)* — `/dk:verify:eval <NN>`
...
triggers the first, a developer-facing surface the second; a milestone with both runs both.
...
**b. Security — always runs** — `/dk:final:security`. Open threats block the ship.
...
not alternatives**; running only one leaves half the doc set unwritten.

`/dk:docs:generate`

*(skip only if this milestone shipped no code-level public surface)* — `/dk:docs:api`

*(only if you took the manual path at step 13)* — `/dk:docs:changelog`
...
milestone, not the command's 7-day default.
