...
5. **Recommendation** — decisive **GO / NO-GO / PIVOT / COULD NOT DETERMINE**, with the single most important condition attached (e.g., "GO, but only with a tighter niche and stripped-down feature scope" or "NO-GO unless you can name three people who'd pay this month"). Use **COULD NOT DETERMINE** instead of forcing a GO/NO-GO/PIVOT call whenever the Research step above didn't run (no web search available) or turned up no independent evidence for Demand, Uniqueness, or Competitive reality — name the specific missing evidence (e.g., "COULD NOT DETERMINE — web search was unavailable, so competitive reality and demand are unverified; re-run with search access before committing either way"). A guessed verdict dressed as decisive is worse than an honest admission the evidence isn't in yet.

If GO: define the lean MVP (core features only, scope controlled, fast shipping, feedback loop) and hand off to the office-hours diagnostic or straight into the standard design flow. If NO-GO: suggest the nearest viable pivot or niche. If COULD NOT DETERMINE: state exactly what evidence would resolve it and how to get it (e.g., run Research with web search enabled, or name three prospective customers) before either GO or NO-GO is attempted again.
