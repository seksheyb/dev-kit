...
- **When to use:** Dispatched by the orchestrator/pipeline to fill validation gaps identified for phase requirements.
...
- **Notes:** Implementation files are read-only for this agent; it can only create/modify test files, fixtures, and its VALIDATION.md report.
...
- **Notes:** Overlaps with `security-auditor`'s dependency-scanning fieldwork but is the dedicated specialist for ongoing dependency hygiene rather than a one-time threat-model verification.
...
- **Notes:** Explicitly related to `secure-code-guardian` (implementation-time prevention) as the audit-time counterpart — this skill finds problems, that one prevents them while writing code.
...
- **What it does:** Verifies each threat from a `<threat_model>` by its declared disposition (mitigate/accept/transfer) — grepping for the mitigation pattern at ALL entry points, not just one — runs SAST/secrets/dependency tools plus mandatory manual auth/input/crypto review, rates findings by CVSS-informed severity, and writes SECURITY.md with a Threat Verification table plus prioritized remediation.
...
- **When to use:** Dispatched by the orchestrator/pipeline to audit an implemented phase against its threat model; falls back to a general audit methodology when no threat model exists (and recommends running the `cso` skill first to build one). Also backs the `/security-audit` command.
...
- **Notes:** Explicitly not a proactive threat-modeling tool (that's `cso`) — this agent verifies mitigations that were already declared, and implementation files are strictly read-only to it.
