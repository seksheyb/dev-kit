...
Display:
```
BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Render the full report, then prompt for approval using an approve/revise/abort or yes/no choice before writing. If the user aborts, exit cleanly with a cancellation message. A synthesis-only agent that does not itself prompt the user (e.g. one that hands its report to a downstream consumer) should instead surface the WARNING count in its return summary so the consumer knows to gate on it.

**If the report is empty (no entries in any bucket):**

Proceed silently or display `No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
