...
# 10. 5-level ladder: a risk sum of 9 (sensitivity:critical=3 + reversibility:destructive=2 +
# tests:none=2 + ambiguity:high=2 + blast=0) now bands to xhigh (min 8, max 9) — the old 3-level
# ladder could not express anything above "high" at all, so this is a genuinely new reachable tier,
# not a shifted boundary. The declared effort ("high") under-declares against it and must fail the
# gate; the declared model ("opus") matches the capability floor ambiguity:high forces, so only the
# effort axis is under test here.
cat > xhigh.md <<'EOF'
## Parallel Execution Map

| Wave | Track | Plan | Depends On | Files Owned | Model | Effort |
|------|-------|------|------------|-------------|-------|--------|
| 1 | track-nuke-session | 01-01 | none | `src/auth/destroy_session.ts` | opus | high |

**Track complexity**

- `track-nuke-session` — complexity: files: src/auth/destroy_session.ts; novelty: low; logic: medium; ambiguity: high; tests: none
EOF
...
run xhigh.md --gate --json
check "risk 9 computes to the new xhigh tier" "$(grep -q '"effort": "xhigh"' <<<"$OUT" && echo 0 || echo 1)"
check "an under-declared 'high' against a computed xhigh fails the gate" "$([ "$RC" = 1 ] && echo 0 || echo 1)" "rc=$RC"
...
# A config copied before the model router landed has neither effortParamSupport nor
# onInexpressibleEffort — validateConfig requires both, so the scorer must inject the plugin
# defaults and warn once on stderr, rather than dying (missing effortBands/modelBands still dies,
# as covered above; this is specifically the two router-only keys).
mkdir -p .claude/bin
cat > .claude/bin/complexity.config.json <<'EOF'
{
  "sensitivePaths": {
    "critical": ["**/auth/**", "**/payments/**", "**/billing/**", "**/security/**", "**/*secret*", "**/*credential*", "**/*token*"],
    "sensitive": ["**/migrations/**", "**/infra/**", "**/terraform/**", "**/db/**", "**/.github/workflows/**"],
    "adjacent": ["**/api/**", "**/middleware/**"]
  },
  "reversibility": {
    "destructive": ["**/*drop*", "**/*destroy*", "**/*delete*", "**/*backfill*", "**/*purge*"],
    "schema": ["**/migrations/**", "**/*schema*"]
  },
  "modelBands": [
    { "min": 0, "max": 3, "model": "haiku" },
    { "min": 4, "max": 7, "model": "sonnet" },
    { "min": 8, "max": 12, "model": "opus" }
  ],
  "effortBands": [
    { "min": 0, "max": 3, "effort": "low" },
    { "min": 4, "max": 7, "effort": "medium" },
    { "min": 8, "max": 12, "effort": "high" }
  ],
  "capabilityFloors": [
...
EOF
run clean.md --gate --json
check "a legacy config (no effortParamSupport/onInexpressibleEffort) still runs" "$([ "$RC" = 0 ] && echo 0 || echo 1)" "rc=$RC"
check "a legacy config prints the migration warning on stderr" \
  "$(grep -q 'config predates the model router' <<<"$OUT" && grep -q '/dk:bootstrap:init' <<<"$OUT" && echo 0 || echo 1)"
check "a legacy config still reports complexity_ok true for a clean plan" "$(grep -q '"complexity_ok": true' <<<"$OUT" && echo 0 || echo 1)"
rm -rf .claude
