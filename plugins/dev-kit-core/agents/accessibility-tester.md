...
## Output
A findings report: each violation with WCAG criterion, severity (critical/serious/moderate/minor), affected location, and a specific fix. Where a criterion could not be verified (e.g., assistive-tech coverage unavailable in this session), list it as its own entry marked **could not determine** — do not omit it or fold it silently into a pass. Lead with critical blockers. Prioritize quick wins, then structural fixes. State the conformance level achieved and any residual limitations. Do not fix code unless the dispatcher asks — report and recommend.
