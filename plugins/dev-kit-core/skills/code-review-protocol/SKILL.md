...
Dispatch in one message, all read-only, all against the frozen SHA:
...
### Stage D — Re-find (parallel, narrowed)

Re-run only the lenses whose findings the sweep actually touched. Re-running all five to confirm a
CSS fix spends a full round to learn nothing. `code-review-gate` is the exception — `stop_loop`
comes from it, so it runs every round regardless.
