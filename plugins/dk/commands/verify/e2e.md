...
Count **surfaces**, not tracks — a project spanning both takes the top row, one surface dispatches `gate-automation` inline and unscoped for phase <NN> on <branch>; `scriptPath` takes a real filesystem path, so run the bare command `dev-kit-core-root` and substitute its output for `<dev-kit-core>`, and a dead run resumes via `Workflow({ scriptPath, resumeFromRunId: "<runId>" })`.
