...
>
> This document defines the CODING profile vocabulary; `references/model-routing.md` defines
> the abstract router contract above it (profiles for research/review/writing/ops, the descriptor,
> surfaces, expressibility) and is the canonical routing procedure for dispatch sites. Planners
> keep emitting exactly the signal blocks defined here.
...
| `xhigh` | Adversarial-grade reasoning — the output feeds a blocking gate or an irreversible/destructive step; enumerate failure modes before concluding. |
| `max` | Maximum rigor for the rare track where a wrong answer is both catastrophic and hard to detect — security-critical, data-destructive, or milestone-gating work at high ambiguity. |
