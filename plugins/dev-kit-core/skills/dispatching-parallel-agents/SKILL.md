...
**Planned, deterministic, fixed-roster fan-outs now route to `Workflow` scripts** under
`references/workflows/` (see that directory's README for the routing convention, prompt
strategies, and the full script index) — that's where `sprint-execution` and `bugfix-wave`
above actually live now, alongside a dozen other pipeline fan-outs. This skill remains the
right tool for the no-ceremony, no-worktree case: ad hoc, mid-session dispatch over
independent investigations that were never written down as a plan or roster.
