...
- **Model selection when the plan doesn't specify:** use the least powerful model that can handle the role. Mechanical 1-2 file tasks with complete specs → cheapest tier. Multi-file integration → standard tier. Architecture/judgment and the final whole-branch review → most capable tier. Turn count beats token price: the cheapest models take 2-3× the turns on multi-step work, so use a mid-tier floor for reviewers and prose-spec implementers.
