...
**Check for existing DESIGN.md** (`docs/global/design/DESIGN.md`):

```bash
ls docs/global/design/DESIGN.md 2>/dev/null || echo "NO_DESIGN_FILE"
```
...
non-interactively, default to Sonnet without blocking (protocol Step 1). That answer, plus every
variant's fully-rendered subagent prompt (Claude Design project id/binding, the concept's approved
brief or tokens, `.dc.html` delivery steps, the verify loop, the self-gate check, and the exact
deliverable contract — `{ variant, file, status: "ok"|"blocked", open_url, blocked_reason?,
notes? }`, `open_url` required when `status` is `"ok"`), is composed in this turn, before dispatch.
Same quality bar as the Phase 5 preview.

- **Exactly 1 confirmed concept:** this is not Shotgun mode after all — dispatch that one
  subagent inline with a plain Agent call.
- **2 or more confirmed concepts:** dispatch is mandatory via the workflow script — first, per
  ...
  operator declined to pick).
  ```js
  Workflow({ scriptPath: "<dev-kit-core>/references/workflows/design-variants.workflow.mjs", args: {
  ...
  and which returned nothing at all (`dropped`). Treat a variant in `blocked` or `dropped` as
  not ready for the comparison board — re-dispatch it before presenting, or present the run with
  that concept plainly missing.

If an image-generation design tool is wired in, it may be used instead of the workflow — that's
optional, not required. Once dispatch returns, build a simple side-by-side **comparison board**
yourself: one `.dc.html` embedding every clean variant (iframes or screenshots) with labels, in
the same Claude Design project. This join — deciding what "clean enough to compare" means, laying
out the board, and deciding how to present any blocked/dropped concepts — stays in this turn; the
...
Write `docs/global/design/DESIGN.md` with this structure (values grounded in the approved
...
| [today] | Claude Design system-creation prompt generated | Only if no system was bound — see `docs/state/tmp/claude-design-system-prompt.md`; awaiting user to run it in Claude Design and report back the resulting id |
...
```markdown
## Design System
Always read docs/global/design/DESIGN.md before making any visual or UI decisions.
