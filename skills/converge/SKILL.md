---
name: converge
description: Assess the current codebase against the feature's spec, plan, and tasks, then append any remaining unbuilt work as new traceable tasks to tasks.md so implementation can complete it. Use when the user says "converge", "diff the code against the spec", "what's left to build", "gap analysis vs spec", "check if the implementation matches the spec", or after an implementation pass to find remaining work.
---
...
Close the gap between what a feature's specification, plan, and tasks call for and what
the codebase currently implements. Read `spec.md`, `plan.md`, and `tasks.md` as the
**sole source of intent** (with the constitution as governing constraints), assess the
current state of the code, determine which requirements, acceptance criteria, plan
decisions, and existing tasks are unmet, incomplete, or only partially satisfied, and
**append each piece of remaining work as a new, traceable task** at the bottom of
`tasks.md` so that an implementation pass can complete it. This should run only after
implementation has run on the current `tasks.md`, and after task generation has produced
a complete `tasks.md`.
...
`## Phase N: Convergence` section to `tasks.md`. You MUST NOT:
...
- modify, create, or delete any application code — completing the appended tasks is the
  job of the implementation pass.

When the codebase already satisfies everything, leave `tasks.md`
...
naming the prerequisite step to run (the `specify` skill for a missing spec, planning for
a missing plan, task generation for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running an implementation pass to complete them; note that a follow-up converge run will
  find fewer or no remaining items.
- On `converged`: recommend proceeding to review / opening a PR. No further implementation
