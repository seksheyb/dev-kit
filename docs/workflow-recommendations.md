...
as-is for the complete audit trail): the capability-level homing table with **Workflow**
as an explicit 5th option, and the full 193-asset sweep flagging which dev-kit
skills/agents/commands should generate/invoke a real Workflow script. Only items needing
a change are listed in both.
...
| 4 | Multi-level requirement hierarchy (Theme→Pillar→Story-bank, global US-xxx) | **Agent/Skill** (optional) | Dev-kit's spec→roadmap→plan→summary chain already gives traceability at project scope; a different ambition (portfolio-scale), only worth building if needed |
| 5 | Vertical-slice enforcement (never horizontal-layer phases) | **Agent/Skill** | `roadmapper.md` already has this as prose; promote to a hard rule + reference doc, same pattern as `planner.md`'s MVP mode |
...
| 9 | Deterministic model/effort scoring — what signals to extract | **Agent/Skill** | An agent stage (in `planner`) extracts raw signals (novelty, files touched, blast radius) as structured output for the Workflow script to sum/band |
...
| 23 | Independent (non-Claude) review bridge — the generic rule | **Agent/Skill** | `gate-plan-review.md` should state "dispatch a non-Claude model for independence" generically, not hardcode `cc-gemini-plugin:gemini` |
...
2. **`gate-codex-round`** — express the ≤6-round adversarial loop as a Workflow `while`
   loop instead of a prose "loop ≤6 times, check majority-refuted" instruction.
...
4. **`gate-plan-review`** — generalize the hardcoded `cc-gemini-plugin:gemini` independence
   check to "dispatch a non-Claude model, via whichever mechanism the environment
   supplies," noting a Workflow `agent()` stage with Bash access is one valid way to host
   that dispatch/retry loop, not the only one.

---

## 2. Workflow-candidate sweep — all 193 dev-kit assets

Every `SKILL.md`, agent, and command file across all 8 plugins (51 core skills, 38 core
agents, 8 core commands, 26 backend, 11 web, 4 mobile, 13+4 data-ai, 14 infra, 19
specialized, 5 product = 193 files), checked for: parallel fan-out over independent items,
an iterative loop with a hard exit condition, or a multi-stage pipeline where later stages
depend on earlier ones completing.

**29 of 193 flagged.** The other 164 (163 no-change + `feature-forge` as Agent-only) need
...
### dev-kit-core agents + commands (12 of 46)
...
| `gate-codex-round` | Agent | Spawned per round of a max-6-round adversarial loop; emits JSON for an external caller to parse and re-invoke | **Workflow** | Loop-up-to-N-with-hard-exit, removes risk of the caller mis-reading `next_action` |
...
*Checked, not flagged:* `code-reviewer`, `doc-verifier`, `health-reporter`, `incident-responder`, `retro`, `roadmapper`, `phase-researcher`, `market-researcher`, `debugger` (needs live user interaction — unsuited to Workflow by design), `gate-plan-review`, `gate-reverse-engineer`.
