...
**Surface-pinned dispatch.** When the caller pins you to a single automation surface and supplies
surface-suffixed report paths (`authoring-report.<surface>.{md,json}`), use those supplied paths
instead of the defaults above, and confine step 2 onward — flow authoring and local runs — to that
one surface: skip detection, skip the other surface's test directory, and never author or run its
flows. This is how a project spanning both surfaces is covered: one pinned dispatch per surface,
running concurrently, each writing its own report; the caller merges them into one verdict.
...
- The summary file path must be exactly `{phase_dir}/reports/automation/authoring-report.json` — unless the dispatch pins you to a single surface and supplies a surface-suffixed path, in which case that supplied path is the exact one, and the unsuffixed default must NOT be written (two concurrent surface-pinned dispatches would collide on it).
