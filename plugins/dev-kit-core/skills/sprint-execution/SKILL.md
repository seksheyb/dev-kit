...
2. **Verify the merge mechanically:** `git log <integration-branch>..<branch> --oneline` must come back **empty** for every branch. A handover's `Merged` field is a self-report, not evidence; this check is the evidence. If any branch still carries commits, stop — do not clean up and do not advance. An unmerged branch left behind means Wave N+1 builds on incomplete work, which is the failure this whole design exists to prevent.
...
- Accept a track's `Merged` field as proof it merged — verify with `git log <integration-branch>..<branch>` (§6.2)
